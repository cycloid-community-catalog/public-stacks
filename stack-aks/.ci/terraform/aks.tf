...
# the following `network` module and change all `module.network.` references to
...
module "aks" {
  ...
  node_network_subnet_id = module.network.vnet_subnet_ids["nodes"]
...
module "aks-node" {
  ...
  #+ ANetwork subnet ID that should be used by AKS nodes.
  network_subnet_id = module.network.vnet_subnet_ids["nodes"]
